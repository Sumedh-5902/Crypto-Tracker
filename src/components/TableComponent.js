...
const SaveBtn = ({data}) => {
  ...
  return(
    ...
    >
                        <svg
                          className={`w-[1.5-rem] ml-1.5
                          ${allCoins.includes(data.id) ? "fill-yellow" : "fill-gray-100"}
                          hover:fill-yellow`}
                          width="30"
                          height="30"
                          viewBox="0 0 30 30"
...
// creating a table and components and details in it
  ...
  return (
    <>
      <div className="flex flex-col mt-9 border border-gray-100 rounded-sm">
        ...
        "
                  >
                    {/* Adding table data cells */}
                    <td className="py-4 flex items-center uppercase ">
                      <SaveBtn data={data} />
                      {/* creating a button for favourite or saved coins */}
                      
                      <img
                        className="w-[1.8rem] h-[1.8rem] mx-1.5"
                        src={data.image}
                        alt={data.name}
                      ></img>
                      <span className="mx-1">
                    ...
                    {/* Filling the data from the CoinGecko API */}
                    <td className="py-4">
                      <Link to={`/${data.id}`} className="cursor-pointer">
                        {data.name}
                      </Link>
      ...
      <div className="flex items-center justify-between mt-4 capitalize h-[2rem]">
        <span>Data provided by <a className="text-yellow" href="www.coingecko.com" rel="noreferrer" target={"blank"}>CoinGecko</a></span>
        <Pagination />
      </div>
    </>
  );
